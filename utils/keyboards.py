...
def kb_account_detail(phone: str):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🗑 𝗗𝗘𝗟𝗘𝗧𝗘 𝗔𝗖𝗖𝗢𝗨𝗡𝗧 🗑️", callback_data=f"delete_account:{phone}")],
...
def kb_target_menu(has_target: bool):
    buttons = []
    if has_target:
        buttons.append([InlineKeyboardButton("🗑 𝗗𝗘𝗟𝗘𝗧𝗘 𝗧𝗔𝗥𝗚𝗘𝗧 🗑️", callback_data="delete_target")])
    else:
